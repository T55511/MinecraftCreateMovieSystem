...
-- =========================
-- 3) チェックリストテンプレ（上書き可）
-- Models.py: checkitem_id は UUID だが server_default が無いので DEFAULT は付けない
-- =========================
CREATE TABLE IF NOT EXISTS tpl_checkitem (
...
-- =========================
-- 2) サブタスクテンプレ（版管理）
-- Models.py: subtask_template_id は UUID だが server_default が無いので DEFAULT は付けない
-- =========================
CREATE TABLE IF NOT EXISTS tpl_subtask_template (
...
-- =========================
-- 1) プロジェクトテンプレ（版管理）
-- Models.py: project_template_id は UUID だが server_default が無いので DEFAULT は付けない
-- =========================
CREATE TABLE IF NOT EXISTS tpl_project_template (
...
-- =========================
-- Event log（※この版はSQLのUUIDのまま）
-- =========================
-- Event log（Models.py に厳密一致）
CREATE TABLE IF NOT EXISTS t_event_log (
  id           BIGSERIAL PRIMARY KEY,
